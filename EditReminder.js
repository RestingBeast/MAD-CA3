...
export function SelectDate({currentDate, selectedTime, sendDate}){
...
export function SelectTime({currentTime, selectedDate, sendTime}){
...
export default function EditReminder({ navigation, route }) {
  ...
  const {currTitle, currDate, currTime, id} = route.params
  const currDateTime = new Date(currDate + " " + currTime)
  const [date, getDate] = useState(currDateTime)
  const [time, getTime] = useState(currDateTime)
  ...
  const editValidatedReminder = () => {
    ...
    // new Date(moment(Date.now()).add(8, 'h').add(30, 'm'))
      ...
      updateReminderById(id, title, date, time)
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topSection}>
        <Text style = {{fontSize: 37, color: 'black', textAlign: 'center'}}>Edit REMINDER</Text>
      </View>
      <View style = {styles.bottomSection}>
        <SelectDate currentDate={moment(currDateTime).format('DD/MM/yyyy')} selectedTime={time} sendDate={getDate}/>
        <SelectTime currentTime={currTime} selectedDate={date} sendTime={getTime}/>
        ...
        <View style = {{flexDirection: "row", marginTop: 60, justifyContent: 'space-evenly'}}>
          <TouchableOpacity onPress={editValidatedReminder} style = {{
